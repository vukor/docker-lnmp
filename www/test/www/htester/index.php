...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (array_key_exists('type', $_POST) && $_POST['type'] == 'mail') {
		if (empty($_SESSION['htester']['mail']['transport'])) {
			$_SESSION['htester']['mail'] = array('transport' => 'sendmail', 'params' => '', 'message' => '', 'error' => '', 'email' => '',);
		}
		...
		elseif ($_SESSION['htester']['mail']['transport'] == 'smtp' && empty($_SESSION['htester']['mail']['params'])) {
			...
			if ($_SESSION['htester']['mail']['transport'] == 'sendmail') {
				$r = mail($email, $subject, $body, '', (!empty($_SESSION['htester']['mail']['params']) ? $_SESSION['htester']['mail']['params'] : ''));
				if ($r) {
...
if (!empty($_SESSION['htester']['mail'])) {
	...
	$mail = array('transport' => 'sendmail', 'params' => '', 'message' => '', 'error' => '', 'email' => '',);
}
...
function item($main, $more = '', $type = '') {
	echo '<li class="' . $type . '"><p class="main">' . $main . '</p><p class="more">' . $more . '</p></li>' . PHP_EOL;
}
